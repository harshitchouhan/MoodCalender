...
dates.forEach((date, idx) => {
  ...
  const dateEl = createDateEl(date, idx);
...
circles.forEach((circle, i) => {
  circle.addEventListener('click', () => {
    ...
    window.localStorage.setItem(`circle-${i}`, circle.style.backgroundColor);
  });

  if (circle.id === `${i}`) {
    circle.style.backgroundColor = window.localStorage.getItem(`circle-${i}`);
  }
});
...
function createDateEl(date, idx) {
  const day = date.getDate();
  const dateEl = document.createElement('div');
  dateEl.classList.add('days');
  dateEl.innerHTML = `<span class="circle" id=${idx}>${day}</span>`;
